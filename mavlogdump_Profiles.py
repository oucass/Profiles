...
Original code from CASS/pymavlink heavily editted to fit this project
'''
...
parser.add_argument("-f", "--follow", action='store_true',
                    help="keep waiting for more data at end of file")
parser.add_argument("--condition", default=None,
                    help="select packets by condition")
parser.add_argument("-q", "--quiet", action='store_true',
                    help="don't display packets")
parser.add_argument("-o", "--output", default=None,
                    help="output matching packets to give file")
parser.add_argument("-p", "--parms", action='store_true',
                    help="preserve parameters in output with -o")
parser.add_argument("--format", default=None,
                    help="Change the output format between 'standard', " +
                    "'json', and 'csv'. For the CSV output, you must " +
                    "supply types that you want.")
parser.add_argument("--csv_sep", dest="csv_sep", default=",",
                    help="Select the delimiter between columns for the" +
                    "output CSV file. Use 'tab' to specify tabs. Only" +
                    " applies when --format=csv")
parser.add_argument("--types", default=None,
                    help="types of messages (comma separated with wildcard)")
parser.add_argument("--nottypes", default=None,
                    help="types of messages not to include " +
                    "(comma separated with wildcard)")
parser.add_argument("--dialect", default="ardupilotmega",
                    help="MAVLink dialect")
parser.add_argument("--zero-time-base", action='store_true',
                    help="use Z time base for DF logs")
parser.add_argument("--no-bad-data", action='store_true',
                    help="Don't output corrupted messages")
parser.add_argument("--show-source", action='store_true',
                    help="Show source system ID and component ID")
parser.add_argument("--show-seq", action='store_true',
                    help="Show sequence numbers")
parser.add_argument("--show-types", action='store_true',
                    help="Shows all message types available on opened log")
parser.add_argument("--source-system", type=int, default=None,
                    help="filter by source system ID")
parser.add_argument("--source-component", type=int, default=None,
                    help="filter by source component ID")
parser.add_argument("--link", type=int, default=None,
                    help="filter by comms link ID")
parser.add_argument("--mav10", action='store_true',
                    help="parse as MAVLink1")
parser.add_argument("--json_out_dir", action='store', dest="json_out_dir",
                    help="output tag for json format")
...
def process(args):
    ...
    output = None
    if args.output:
        output = open(args.output, mode='wb')  # JESSICA
    ...
    if istlog and args.format == 'csv':  # we know our fields from the get-go
        try:
            ...
            print("You must specify a list of message types if outputting CSV"
                  + " format via the --types argument.")
            exit()
    ...
    if isbin and args.format == 'csv':  # need to collect columns from message
        if types is None or len(types) != 1:
            print("Need exactly one type when dumping CSV from bin file")
            quit()

    # Track the last timestamp value.
    # Used for compressing data for the CSV output format.
    last_timestamp = None
    ...
    # Keep track of data from the current timestep. If the following timestep
    # has the same data, it's stored in here as well. Output should therefore
    # have entirely unique timesteps.
    while True:
        ...
        if not mavutil.evaluate_condition(args.condition, mlog.messages):
            continue
        if(args.source_system is not None and
           args.source_system != m.get_srcSystem()):
            continue
        if(args.source_component is not None and
           args.source_component != m.get_srcComponent()):
            continue
        if args.link is not None and args.link != m._link:
            continue

        if(types is not None and m.get_type() != 'BAD_DATA'
           and not match_type(m.get_type(), types)):
            continue

        if nottypes is not None and match_type(m.get_type(), nottypes):
            continue

        # Ignore BAD_DATA messages is the user requested or if they're because
        # of a bad prefix. The
        # latter case is normally because of a mismatched MAVLink version.
        if(m.get_type() == 'BAD_DATA' and
           (args.no_bad_data is True or m.reason == "Bad prefix")):
            continue

        # Grab the timestamp.
        timestamp = getattr(m, '_timestamp', 0.0)

        # If just logging, pack in the timestamp and data into the output file.
        if output:
        ...
        # If JSON was ordered, serve it up. Split it nicely into metadata
        # and data.
        if args.format == 'json':
        ...
        # CSV format outputs columnar data with a user-specified delimiter
            ...
            if timestamp == last_timestamp or last_timestamp is None:
                if isbin:
                    newData = [str(data[y]) if y != "timestamp" else ""
                               for y in fields]
                else:
            ...
            # Otherwise if this is a new timestamp, print out the old output
            # data, and store the current message for later output.
            else:
                csv_out[0] = "{:.8f}".format(last_timestamp)
                print(args.csv_sep.join(csv_out))
                if isbin:
                    csv_out = [str(data[y]) if y != "timestamp"
